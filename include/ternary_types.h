...
/* Trit 枚举：拓扑态的三种相位
 *
 * [统一规范 2026-08-16] 平衡 {−1,0,+1} 仅作为指令核心内部计算域编码。
 * 存储/域运算一律采用主权标准三进制 {0,1,2}:
 *   TRIT_NEG(−1) ↔ 标准 0,  TRIT_ZERO(0) ↔ 标准 1,  TRIT_POS(+1) ↔ 标准 2
 * 边界转换 (bal = std − 1, 精确整数): 见 gf9_ops.h 的
 *   sov3_to_balanced / balanced_to_sov3 (BitNet 权重域映射)。
 */
typedef enum {
